...
--
-- supplier_code here is the ORACLE supplier code used on receiving_orders
-- (the qr_template join goes receiving_orders.supplier_code -> suppliers.code
-- -> supplier_profiles.supplier_code), NOT the text brand code on parts.
-- Mapping evidence: brand->supplier from prod receiving lines (KOA->32,
-- NCC->23) and user decision for the rest.
--
-- barcode_types = PDA hardware-scanner symbology whitelist (xcheng display
-- names, see docs/superpowers/specs/2026-09-04-supplier-barcode-type-whitelist-design.md).
-- NULL = no restriction. COPAL is left NULL (label symbology unconfirmed).
--
-- 10 old RegPattern rows collapse to 6 templates / 7 rows:
--   32    KOA ELECTRONICS (HK)        <- Id=161 (canonical 'Primary Key' row);
--                                      Id=192 KOA_NOLOTNO / Id=193 KOA_NOTKEY
--                                      are subsets of the same label layout.
--   23    HONGKONG CHEMI-CON (NCC)    <- Id=165 + Id=167 NCC+KOA (identical
--                                      28-char layout) + Id=195 NCC_KTD
--                                      (32-char; folded into one regex).
--   19915 NIDEC(H.K.) (COPAL)         <- Id=156 (fixed-width 111-char reel label)
--   70915 SII SEMICONDUCTOR HK        <- Id=173 ($-delimited reel QR)
--   84915 ABLIC HONG KONG             <- same template (SII renamed ABLIC)
--   15915 TE CONNECTIVITY HK          <- Id=194 (MH10.8.2 DataMatrix; photos)
--   69915 NDK ELECTRONICS (HK)        <- Id=170 NDK_16M (MH10.8.2 QR; photos)
--
...
--
-- Idempotent: upserts on supplier_code. qr_template_config is intentionally
-- left NULL (hand-written legacy templates).
-- ---------------------------------------------------------------------------
...
VALUES
  ...
  -- COPAL (supplier 19915, NIDEC(H.K.)CO.,LIMITED): fixed-width 111-char reel
  -- label, e.g.
  ...
  -- SII (supplier 70915, SII SEMICONDUCTOR HONG KONG): $-delimited reel QR,
  -- e.g. 1$S-5743NBH1A-M3T4U$3000$VM3F6XA1761-0367$S-5743NBH1A-M3T4U$202607$67352039027
  ...
  -- The scanned parts exist in the parts master under brand 'ABLIC' (SII was
  -- renamed ABLIC); the same template is also attached to supplier 84915
  -- (ABLIC HONG KONG LTD.) below.
  ...
  -- ABLIC (supplier 84915, ABLIC HONG KONG LTD.): same SII template as 70915.
  (
  ...
  -- TE Connectivity (supplier 15915, TE CONNECTIVITY HK LIMITED): MH10.8.2
  -- DataMatrix, e.g. (GS=\x1d, RS=\x1e, EOT=\x04)
  ...
  -- NDK (supplier 69915, NDK ELECTRONICS (HK) LTD.): MH10.8.2 QR on the
  -- reel/box label, e.g.
...
SELECT supplier_code, qr_type, qty_encoding, barcode_types, left(qr_template, 60) AS template_head
FROM supplier_profiles
WHERE supplier_code IN ('32', '23', '19915', '70915', '84915', '15915', '69915')
