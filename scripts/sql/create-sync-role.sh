...
BEGIN
  IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'warehouse_sync') THEN
    ...
    CREATE ROLE warehouse_sync LOGIN SUPERUSER CREATEDB INHERIT REPLICATION NOBYPASSRLS PASSWORD '${SYNC_DB_PASSWORD:-warehouse_sync}';
  ELSE
    ALTER ROLE warehouse_sync SUPERUSER CREATEDB INHERIT LOGIN REPLICATION NOBYPASSRLS;
  END IF;
END
\$\$;
SQL
