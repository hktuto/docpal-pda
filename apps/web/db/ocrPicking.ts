...
import { scanAllocationToPackage } from "./picking";
  ...
  const result = await db
    .execute(sql`
      ...
      SELECT
        ...
        (rii.received_qty - rii.picked_qty - rii.put_away_qty - COALESCE(alloc.allocated_qty, 0) - COALESCE(alloc.unboxed_scanned_qty, 0)) AS available_qty
      ...
      WHERE ro.id = ${receivingOrderId}
        ...
        AND (rii.received_qty - rii.picked_qty - rii.put_away_qty - COALESCE(alloc.allocated_qty, 0) - COALESCE(alloc.unboxed_scanned_qty, 0)) >= ${qty}
      ...
      SELECT DISTINCT
        ...
        COALESCE((
          ...
          WHERE pp.picking_item_id = pi.id
            AND pp.shipping_box_id IS NULL
...
export async function findReceivingCandidatesForOrder(
  ...
  const result = await db
    .execute(sql`
      ...
      WHERE ro.id = ${receivingOrderId}
        AND ro.status = 'in_hand'
        AND (rii.received_qty - rii.picked_qty - rii.put_away_qty - COALESCE(alloc.allocated_qty, 0) - COALESCE(alloc.unboxed_scanned_qty, 0)) > 0
...
export async function applyOcrPick(
  ...
  return db.transaction(async (tx) => {
    ...
    const invoiceItems = await tx.execute(sql`
      WITH allocations_cte AS (${allocationsCte(pickingItemId)})
      SELECT
        rii.id AS receiving_invoice_item_id,
        rii.received_qty,
        rii.picked_qty,
        rii.put_away_qty,
        rii.date_code,
        COALESCE(alloc.allocated_qty, 0) AS reserved_by_others,
    ...
    // Compute FIFO invoice-item portions for the scan.
    const portions: { receivingInvoiceItemId: string; qty: number }[] = [];
    ...
    for (const raw of invoiceItems.rows ?? []) {
      ...
      portions.push({ receivingInvoiceItemId: String(raw.receiving_invoice_item_id), qty: use });
      remainingScan -= use;
    }
    ...
    // Consume the coarse receiving-order allocations for this scan.
    let allocationQtyToConsume = qty;
    let allocationIndex = 0;
    while (allocationQtyToConsume > 0) {
      const allocation = allocations[allocationIndex];
      if (!allocation) throw new I18nError("allocation_not_found");
      const take = Math.min(allocationQtyToConsume, allocation.qty);
      if (take < allocation.qty) {
        await tx
          .update(schema.allocations)
          .set({ qty: sql`${schema.allocations.qty} - ${take}` })
      ...
      allocationQtyToConsume -= take;
      allocationIndex++;
    }

    // Create one receiving-area lot for the whole scan with sources pointing to the consumed invoice items.
    const lotId = uuid();
    ...
    for (const portion of portions) {
      await tx.insert(schema.inventoryLotSources).values({
        ...
        receivingInvoiceItemId: portion.receivingInvoiceItemId,
        qty: portion.qty,
      });
    }

    // Create a single allocation tied to the lot and scan it into one package.
    const lotAllocationId = uuid();
    await tx.insert(schema.allocations).values({
      id: lotAllocationId,
      pickingItemId,
      inventoryLotId: lotId,
    ...
    await scanAllocationToPackage(db, lotAllocationId, qty, actorId, tx);
  });
}
