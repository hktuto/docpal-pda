...
// renders a list row's title/meta through the warehouse-configured per-list
// templates (flow-config key pdaListTemplates, resolved via GET /config
// listTemplates). A placeholder whose field is empty renders as "" (no
// dangling separator); arrays comma-join; unknown [tokens] stay literal. An
// all-empty TITLE falls back to the default title template, then to the raw
// primary id; an all-empty META returns "" and the caller hides the line.
// This file is framework-free — apps/admin keeps a mirror copy for the
// display-config live preview.
