...
function insert(table: string, columns: string[], values: unknown[]): string {
  return `INSERT INTO ${table} (${columns.join(", ")}) VALUES (${values.map(lit).join(", ")});`;
}
  ...
  it("writes apps/api/src/db/seedSql.ts projected onto the API schema", async () => {
    ...
    try {
      ...
      const select = async (table: string) =>
        (await pg.query(`SELECT * FROM ${table}`)).rows as Record<string, unknown>[];
      ...
      const shelves = await select("shelves");
      ...
      for (const r of pickingOrders) {
        lines.push(
          insert(
            "picking_orders",
            ["id", "external_id", "ref_no", "status", "ship_to", "destination_country", "issue_reason", "issue_note", "created_at", "updated_at"],
            [r.id, r.ref_no, r.ref_no, r.status, r.ship_to, r.destination_country, r.issue_reason, r.issue_note, iso(r.created_at), iso(r.updated_at)]
          )
        );
      }
      ...
      const escaped = sql.split("`").join("\\`").split("${").join("\\${");
