...
export type GoodsVerifyTaskStatus = "pending" | "completed";

export interface GoodsVerifyTaskBox {
  id: string;
  shelfCode: string;
  status: BoxStatus;
  itemCount: number;
...
export interface GoodsVerifyTaskShelf {
  code: string;
  zone: string | null;
  boxes: GoodsVerifyTaskBox[];
}

export interface GoodsVerifyTaskSummary {
  id: string;
  refNo: string;
  status: GoodsVerifyTaskStatus;
  createdAt: string;
  dueDate: string;
  shelfCount: number;
  boxCount: number;
