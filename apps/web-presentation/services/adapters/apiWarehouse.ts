...
function toPickingByReceivingRow(row: RawRow): PickingByReceivingRow {
  return {
    ...
    coo: row.coo ?? null,
    cow: row.cow ?? null,
    box_remark: row.box_remark ?? null,
...
export function createApiWarehouseService(
  ...
  return {
    ...
    async getGoodsVerifyTasks() {
      // Not used in API demo mode; presentation package uses pglite.
      return [];
    },

    async getGoodsVerifyTask(id) {
      throw new I18nError("goods_verify_task_not_found");
    },
