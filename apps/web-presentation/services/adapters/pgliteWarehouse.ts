...
import type {
  ...
  GoodsVerifyTaskSummary,
  GoodsVerifyTaskDetail,
  GoodsVerifyTaskShelf,
  GoodsVerifyTaskBox,
...
// ------------------------------------------------------------------
// Demo goods-verify task (pre-calculated display data for screen capture)
// ------------------------------------------------------------------
  ...
  boxes: [
    {
      ...
      items: [
        { id: "gv-h3398-1", partNo: "RK73H1ETTP1000F", qty: 1000, verified: false, verifiedAt: null },
        { id: "gv-h3398-2", partNo: "RK73H1ETTP1001F", qty: 500, verified: false, verifiedAt: null },
      ],
    },
    {
      id: "H3402",
      ...
      status: "open" as const,
      items: [
        { id: "gv-h3402-1", partNo: "RK73H1ETTP2001F", qty: 800, verified: false, verifiedAt: null },
      ],
    },
    {
      id: "H3410",
      ...
      status: "open" as const,
      items: [
        { id: "gv-h3410-1", partNo: "RK73H2ATTD10R0F", qty: 1200, verified: false, verifiedAt: null },
        { id: "gv-h3410-2", partNo: "RK73H2ATTD20R0F", qty: 300, verified: false, verifiedAt: null },
      ],
    },
  ] as DemoTaskBox[],
};

function getDemoTaskShelves(): GoodsVerifyTaskShelf[] {
  const map = new Map<string, GoodsVerifyTaskBox[]>();
  for (const box of demoGoodsVerifyTask.boxes) {
    const list = map.get(box.shelfCode) ?? [];
    list.push({
      id: box.id,
      shelfCode: box.shelfCode,
      status: box.status,
      itemCount: box.items.length,
      verifiedCount: box.items.filter((i) => i.verified).length,
    });
    map.set(box.shelfCode, list);
  }
  return Array.from(map.entries()).map(([code, boxes]) => ({
    code,
    zone: code.startsWith("A") ? "Zone A" : "Zone B",
    boxes,
  }));
}

function getDemoTaskSummary(): GoodsVerifyTaskSummary {
  const verifiedBoxCount = demoGoodsVerifyTask.boxes.filter((b) => b.status === "verified").length;
  return {
    id: demoGoodsVerifyTask.id,
    refNo: demoGoodsVerifyTask.refNo,
    status: demoGoodsVerifyTask.status,
    createdAt: demoGoodsVerifyTask.createdAt,
    dueDate: demoGoodsVerifyTask.dueDate,
    shelfCount: getDemoTaskShelves().length,
    boxCount: demoGoodsVerifyTask.boxes.length,
    verifiedBoxCount,
  };
}

function isDemoBox(id: string) {
  return demoGoodsVerifyTask.boxes.some((b) => b.id === id);
}

function isDemoShelf(code: string) {
  return demoGoodsVerifyTask.boxes.some((b) => b.shelfCode === code);
}

function toDemoGoodsVerifyShelfBoxSummary(box: DemoTaskBox): GoodsVerifyShelfBoxSummary {
  const verifiedCount = box.items.filter((i) => i.verified).length;
  return {
    id: box.id,
    shelfCode: box.shelfCode,
    status: box.status,
    itemCount: box.items.length,
    verifiedCount,
    lastCheckAt: verifiedCount > 0 ? new Date() : null,
    checkedToday: verifiedCount > 0,
  };
}

function toDemoGoodsVerifyShelfBoxDetail(box: DemoTaskBox): GoodsVerifyShelfBoxDetail {
  return {
    id: box.id,
    ...
    status: box.status,
    createdAt: new Date(),
    shelf: { code: box.shelfCode, zone: box.shelfCode.startsWith("A") ? "Zone A" : "Zone B" },
    receivingOrder: null,
    items: box.items.map((item) => ({
      ...
      part: { partNo: item.partNo, description: null },
    })),
  };
}

function checkDemoTaskCompletion() {
  const allVerified = demoGoodsVerifyTask.boxes.every((b) => b.status === "verified");
  if (allVerified) {
    demoGoodsVerifyTask.status = "completed";
  }
}
...
export function createPgliteWarehouseService(
  ...
  return {
    ...
    async getShelfBoxes(shelfCode: string): Promise<GoodsVerifyShelfBoxSummary[]> {
      if (isDemoShelf(shelfCode)) {
        return demoGoodsVerifyTask.boxes
          .filter((b) => b.shelfCode === shelfCode)
          .map(toDemoGoodsVerifyShelfBoxSummary);
      }
      return dbGetShelfBoxesByShelf(db, shelfCode);
    },

    async getShelfBox(id: string): Promise<GoodsVerifyShelfBoxDetail> {
      const demoBox = demoGoodsVerifyTask.boxes.find((b) => b.id === id);
      if (demoBox) return toDemoGoodsVerifyShelfBoxDetail(demoBox);
    ...
    async verifyShelfBoxItem(shelfBoxId: string, partId: string): Promise<void> {
      const demoBox = demoGoodsVerifyTask.boxes.find((b) => b.id === shelfBoxId);
      if (demoBox) {
        const item = demoBox.items.find((i) => i.id === partId);
        if (!item) throw new I18nError("shelf_box_item_not_found");
        item.verified = true;
    ...
    async markShelfBoxVerified(id: string): Promise<void> {
      const demoBox = demoGoodsVerifyTask.boxes.find((b) => b.id === id);
      if (demoBox) {
        const allVerified = demoBox.items.every((i) => i.verified);
        if (!allVerified) throw new I18nError("not_all_shelf_box_items_verified");
        demoBox.status = "verified";
        checkDemoTaskCompletion();
        return;
      }
      await dbMarkShelfBoxVerified(db, id, assertActorId(getActorId));
    },

    async getGoodsVerifyTasks(): Promise<GoodsVerifyTaskSummary[]> {
      return [getDemoTaskSummary()];
    },

    async getGoodsVerifyTask(id: string): Promise<GoodsVerifyTaskDetail> {
      if (id !== demoGoodsVerifyTask.id) throw new I18nError("goods_verify_task_not_found");
      return {
        id: demoGoodsVerifyTask.id,
        refNo: demoGoodsVerifyTask.refNo,
        status: demoGoodsVerifyTask.status,
