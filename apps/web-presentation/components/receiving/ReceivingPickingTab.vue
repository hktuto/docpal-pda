    ...
    <div v-for="pi in po.items" :key="pi.id" class="lot" style="margin-top: 0.75rem;">
      ...
      <ul v-if="allocatedLocations(pi).length" style="margin: 0; padding-left: 1.25rem; font-size: 0.875rem; color: var(--muted);">
        <li v-for="(loc, idx) in allocatedLocations(pi)" :key="idx">
          {{ loc.shelf_code || (loc.box_id ? $t('common.inBox', { id: loc.box_id }) : receivingAreaLabel(loc.box_remark)) }}
...
function parseBoxRemark(remark: string | null): string[] {
  if (!remark) return [];
  try {
...
function receivingAreaLabel(boxRemark: string | null): string {
  const base = t("receiving.pickingTab.receivingArea");
  const boxIds = parseBoxRemark(boxRemark);
  // Demo fallback: always show a sample box id in the receiving area allocation line
  const ids = boxIds.length > 0 ? boxIds : ["H3398"];
