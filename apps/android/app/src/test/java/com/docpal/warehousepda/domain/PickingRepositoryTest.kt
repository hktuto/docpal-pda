...
class PickingRepositoryTest {
    ...
    private fun insertBaseFixture() {
        ...
        insertPickingOrder(db, PO, "TEST-PO-01", "pending")
        insertPickingItem(db, PI, PO, PART, 20, allocated = 5)
        exec(
            db,
    ...
    fun `applyOcrPick creates package, lot, sources and consumes allocation`() = runBlocking {
        ...
        assertEquals(1, intQuery(db, "SELECT COUNT(*) FROM picking_packages WHERE picking_item_id = '$PI'"))
        assertEquals(5, intQuery(db, "SELECT qty FROM picking_packages WHERE picking_item_id = '$PI'"))
        assertEquals(
            "inventory_lot",
            stringQuery(db, "SELECT source_type FROM picking_packages WHERE picking_item_id = '$PI'"),
        )
        assertEquals("2406", stringQuery(db, "SELECT date_code FROM picking_packages WHERE picking_item_id = '$PI'"))
        assertEquals("L1", stringQuery(db, "SELECT lot_code FROM picking_packages WHERE picking_item_id = '$PI'"))
        assertEquals("MY", stringQuery(db, "SELECT coo FROM picking_packages WHERE picking_item_id = '$PI'"))
        assertNull(stringQuery(db, "SELECT shipping_box_id FROM picking_packages WHERE picking_item_id = '$PI'"))

        // Lot created with 5, fully consumed by the scan.
        val lotId = stringQuery(db, "SELECT source_id FROM picking_packages WHERE picking_item_id = '$PI'")!!
        assertEquals(0, intQuery(db, "SELECT total_qty FROM inventory_lots WHERE id = '$lotId'"))
        assertEquals(0, intQuery(db, "SELECT allocated_qty FROM inventory_lots WHERE id = '$lotId'"))
        assertEquals(1, intQuery(db, "SELECT COUNT(*) FROM inventory_lot_sources WHERE inventory_lot_id = '$lotId'"))
        assertEquals(0, intQuery(db, "SELECT qty FROM inventory_lot_sources WHERE inventory_lot_id = '$lotId'"))

        // Invoice item picked via the source walk; item picked_qty untouched until boxed.
        assertEquals(5, intQuery(db, "SELECT picked_qty FROM receiving_invoice_items WHERE id = '$INV_ITEM'"))
        assertEquals(0, intQuery(db, "SELECT picked_qty FROM picking_items WHERE id = '$PI'"))
        // allocated 5 (coarse) -> 0 after scan.
        assertEquals(0, intQuery(db, "SELECT allocated_qty FROM picking_items WHERE id = '$PI'"))
        // Coarse allocation fully consumed -> deleted; lot allocation remains at 0.
        assertEquals(0, intQuery(db, "SELECT COUNT(*) FROM allocations WHERE id = '$ALLOC'"))
        assertEquals(
            0,
            intQuery(db, "SELECT qty FROM allocations WHERE picking_item_id = '$PI' AND inventory_lot_id = '$lotId'"),
        )
        ...
        val packageId = stringQuery(db, "SELECT id FROM picking_packages WHERE picking_item_id = '$PI'")!!
        assertTrue(metadata!!.contains(packageId))
    }

    @Test
    fun `applyOcrPick rejects qty above remaining and above availability`() = runBlocking<Unit> {
        ...
        insertPickingOrder(db, PO2, "TEST-PO-02", "pending")
        insertPickingItem(db, PI2, PO2, PART, qty = 100, allocated = 95)
    ...
    @Test
    fun `applyOcrPick rejects wrong part and non in_hand order`() = runBlocking<Unit> {
        ...
        insertPickingItem(db, PI_OTHER, PO, PART_OTHER, qty = 10, allocated = 0)
    ...
    fun `applyOcrPick tops up coarse allocation when scan exceeds existing`() = runBlocking {
        ...
        assertEquals(1, intQuery(db, "SELECT COUNT(*) FROM picking_packages WHERE picking_item_id = '$PI'"))
        assertEquals(10, intQuery(db, "SELECT qty FROM picking_packages WHERE picking_item_id = '$PI'"))
        // Both coarse allocations consumed and deleted.
        assertEquals(
            0,
            intQuery(db, "SELECT COUNT(*) FROM allocations WHERE picking_item_id = '$PI' AND receiving_order_id = '$RO'"),
        )
        // allocated: 5 + 5 (top-up) - 10 (scan) = 0.
        assertEquals(0, intQuery(db, "SELECT allocated_qty FROM picking_items WHERE id = '$PI'"))
        assertEquals(10, intQuery(db, "SELECT picked_qty FROM receiving_invoice_items WHERE id = '$INV_ITEM'"))
    }
    ...
    fun `addPackageToBox increments picked qty and remove reverts it`() = runBlocking {
        ...
        assertEquals(5, intQuery(db, "SELECT picked_qty FROM picking_items WHERE id = '$PI'"))
        assertEquals(boxId, stringQuery(db, "SELECT shipping_box_id FROM picking_packages WHERE id = '$packageId'"))
        assertNotNull(
            stringQuery(
                db,
        ...
        assertEquals("pending", stringQuery(db, "SELECT status FROM picking_orders WHERE id = '$PO'"))

        repo.removePackageFromBox(packageId, ACTOR)
        assertEquals(0, intQuery(db, "SELECT picked_qty FROM picking_items WHERE id = '$PI'"))
        assertNull(stringQuery(db, "SELECT shipping_box_id FROM picking_packages WHERE id = '$packageId'"))
        assertEquals(0, intQuery(db, "SELECT verified FROM picking_packages WHERE id = '$packageId'"))
        assertNotNull(
            stringQuery(
                db,
                "SELECT id FROM transition_logs " +
    ...
    fun `addAllUnboxedPackagesToBox boxes everything and auto-finishes order`() = runBlocking {
        ...
        insertPickingOrder(db, PO2, "TEST-PO-02", "pending")
        insertPickingItem(db, PI2, PO2, PART, qty = 5, allocated = 5)
        ...
        assertEquals("finished", stringQuery(db, "SELECT status FROM picking_orders WHERE id = '$PO2'"))
        assertEquals(5, intQuery(db, "SELECT picked_qty FROM picking_items WHERE id = '$PI2'"))
        // Measuring task created and linked to the order's boxes.
        val taskId = stringQuery(db, "SELECT id FROM measuring_tasks WHERE picking_order_id = '$PO2'")
        assertNotNull(taskId)
        assertEquals("pending", stringQuery(db, "SELECT status FROM measuring_tasks WHERE picking_order_id = '$PO2'"))
        assertEquals(taskId, stringQuery(db, "SELECT measuring_task_id FROM shipping_boxes WHERE id = '$boxId'"))
    ...
    fun `removeScannedPackage reverses scan and restores allocation`() = runBlocking {
        repo.applyOcrPick(RO, PI, 5, "2406", "L1", "MY", null, ACTOR)
        val packageId = stringQuery(db, "SELECT id FROM picking_packages WHERE picking_item_id = '$PI'")!!
        val lotId = stringQuery(db, "SELECT source_id FROM picking_packages WHERE id = '$packageId'")!!

        repo.removeScannedPackage(packageId, ACTOR)

        assertEquals(0, intQuery(db, "SELECT COUNT(*) FROM picking_packages WHERE id = '$packageId'"))
        // Invoice item picked_qty restored.
        assertEquals(0, intQuery(db, "SELECT picked_qty FROM receiving_invoice_items WHERE id = '$INV_ITEM'"))
        // Lot and its source restored.
        assertEquals(5, intQuery(db, "SELECT total_qty FROM inventory_lots WHERE id = '$lotId'"))
        assertEquals(5, intQuery(db, "SELECT allocated_qty FROM inventory_lots WHERE id = '$lotId'"))
        assertEquals(5, intQuery(db, "SELECT qty FROM inventory_lot_sources WHERE inventory_lot_id = '$lotId'"))
        // Lot allocation restored to 5; item allocated_qty restored to 5.
        assertEquals(
            5,
            intQuery(db, "SELECT qty FROM allocations WHERE picking_item_id = '$PI' AND inventory_lot_id = '$lotId'"),
        )
        assertEquals(5, intQuery(db, "SELECT allocated_qty FROM picking_items WHERE id = '$PI'"))
        assertEquals(0, intQuery(db, "SELECT picked_qty FROM picking_items WHERE id = '$PI'"))
        ...
        assertEquals("in_hand", stringQuery(db, "SELECT status FROM receiving_orders WHERE id = '$RO'"))
    }
    ...
    @Test
    fun `validation errors match web keys`() = runBlocking<Unit> {
        // box_is_not_open: package into a cancelled box.
        insertPackage(db, PKG1, PI, PO, qty = 5, boxId = null)
        insertBox(db, BOX1, PO, "cancelled")
        ...
        insertPackage(db, PKG2, PI, PO, qty = 5, boxId = BOX1)
        ...
        insertPickingOrder(db, PO_ISSUE, "TEST-PO-03", "issue")
        insertPickingItem(db, PI_ISSUE, PO_ISSUE, PART, qty = 10, allocated = 0)
        insertPackage(db, PKG3, PI_ISSUE, PO_ISSUE, qty = 5, boxId = null)
        insertBox(db, BOX3, PO_ISSUE, "open")
    ...
    @Test
    fun `materializeReceivingAllocation converts coarse allocation into lot and sources`() = runBlocking<Unit> {
        ...
        assertNull(stringQuery(db, "SELECT receiving_order_id FROM allocations WHERE id = '$ALLOC'"))
        val lotId = stringQuery(db, "SELECT inventory_lot_id FROM allocations WHERE id = '$ALLOC'")
        assertNotNull(lotId)
        assertEquals(5, intQuery(db, "SELECT total_qty FROM inventory_lots WHERE id = '$lotId'"))
        assertEquals(5, intQuery(db, "SELECT allocated_qty FROM inventory_lots WHERE id = '$lotId'"))
        assertEquals(5, intQuery(db, "SELECT qty FROM inventory_lot_sources WHERE inventory_lot_id = '$lotId'"))
        assertEquals(
            INV_ITEM,
            stringQuery(db, "SELECT receiving_invoice_item_id FROM inventory_lot_sources WHERE inventory_lot_id = '$lotId'"),
        )
        ...
        assertEquals(6, intQuery(db, "SELECT qty FROM allocations WHERE id = '$ALLOC2'"))
        assertEquals(RO, stringQuery(db, "SELECT receiving_order_id FROM allocations WHERE id = '$ALLOC2'"))
        assertEquals(4, intQuery(db, "SELECT qty FROM allocations WHERE id = '$newId'"))
        assertNotNull(stringQuery(db, "SELECT inventory_lot_id FROM allocations WHERE id = '$newId'"))
