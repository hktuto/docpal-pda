// Global UI text-size setting (px applied to <html>). All admin CSS sizes are
// rem off this root value. Per-device localStorage, applied at app start by
// plugins/font-size.client.ts. Same storage key as the web PDA app so the
// setting is consistent where origins are shared.
