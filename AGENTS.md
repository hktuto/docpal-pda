...
- **Backend (`apps/backend`):** Hono + Drizzle + PostgreSQL (`postgres` driver, `DATABASE_URL` overrides). Port `3002`, database `warehouse_backend`; tests against `TEST_DATABASE_URL` (default `warehouse_backend_test`). Migrations auto-apply on startup. Backend docs: `docs/backend/`.
  ...
  Top-level `receivingOrderNameTemplate` (default `"[batch_no]"`) — the receiving order NAME template: placeholders `[batch_no]`/`[invoice_no]` (comma-joined invoice numbers)/`[supplier_code]`/`[supplier_name]`/`[delivery_date]` (YYYY-MM-DD)/`[date_code]`; the backend renders it into `displayName` on the `GET /receiving-orders` rows + detail (all-empty render falls back to batch_no), consumed by the PDA receiving list/detail titles and the admin receiving list/detail (spec `docs/superpowers/specs/2026-09-21-receiving-order-name-template-design.md`).
