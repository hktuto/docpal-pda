...
`docker-compose.prod.yml` runs the full stack — db + backend + web (PDA SPA) + admin console — as a separate compose project (`warehouse-prod`) so it coexists with the dev `docker-compose.yml` db on :5432. Per-app Dockerfiles (`apps/{backend,web,admin}/Dockerfile`, build context = repo root, pnpm via corepack): backend is `tsc`-compiled and runs `node dist/server.js` (migrations auto-apply at boot; `src/db/seed-parts-data.json` is copied into `dist/db/` because the seed reads it at runtime); web/admin are `nuxt build` SPAs served by the nitro node server (`node server/index.mjs`) — `apiBaseUrl` is public runtime config, overridable at container start with `NUXT_PUBLIC_API_BASE_URL` (no rebuild). Required env: `POSTGRES_PASSWORD`, `AUTH_SECRET` (compose refuses to start without them); optional: `CORS_ORIGINS` (add the host's LAN origins), `WEB_API_BASE_URL`/`ADMIN_API_BASE_URL`, `FLOW_STEPS_DISABLED`, `WAREHOUSE_SEED=off` (skip the demo seed). Ports: web 3000, admin 3100, backend 3002; the prod db is not published to the host. `DEV_ROUTES=off` is set in prod.
