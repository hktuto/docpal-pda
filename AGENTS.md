...
- **Backend (`apps/backend`):** Hono + Drizzle + PostgreSQL (`postgres` driver, `DATABASE_URL` overrides). Port `3002`, database `warehouse_backend`; tests against `TEST_DATABASE_URL` (default `warehouse_backend_test`). Migrations auto-apply on startup. Backend docs: `docs/backend/`.
  ...
  - **Printing:** `/print/*` (`src/routes/print.ts` + `src/print.ts`) proxies the label-printing-center print service — printer list, template dynamic print, direct file print, job status (no preview). Base URL from `PRINT_API_BASE_URL` (default `http://192.168.5.116:9003`); upstream errors → 502. Upstream API doc: `docs/backend/print-service.md`.
