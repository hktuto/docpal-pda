...
| `GET /admin/picking-orders` | Admin console picking-orders list: same `{rows, total}` shape and `status`/`allocation`/`search`/`limit`/`offset` params as `GET /picking-orders`, but UNSCOPED — ignores the flow config's `allowedOrgIds` and the caller's user sub-inventory scope (`listPickingOrders` `unscoped: true`), so admins see every order. |
| `GET /admin/picking-orders/:id` | Admin console picking-order detail: same nested read as `GET /picking-orders/:id` (`getPickingOrderDetail` `unscoped: true`), ignoring both org filters — orders outside `allowedOrgIds` / the caller's user scope still open in the admin console; 404 `picking_order_not_found` only when the id does not exist. |
