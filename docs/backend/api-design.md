...
| `POST /admin/picking-orders/:id/reallocate` | — → `{allocation}` (scoped summary incl. `partKeys`) | recompute scoped to the order's part keys (`allocateForPickingOrder`); 404 `picking_order_not_found`, 409 `order_not_open` unless pending/picking/allocated (an `allocated` order is force-included as a demand — the explicit repair path for a stale lock), 409 `lock_held` with `{error, holderId, holderName}` when a live PDA work lock is held. Spec `docs/superpowers/specs/2026-09-14-admin-allocation-buttons-design.md` |
...
| `PATCH /admin/picking-orders/:id/status` | `{status, reason?}` → `{id, orderNo, status, previousStatus, changed}` | admin status override (`overridePickingOrderStatus`) to any of `pending`/`allocated`/`skip`/`picking`/`issue`/`finished`/`shipped` — no transition guards; a live PDA work lock is force-cleared; leaving an allocation-holding status (`pending`/`picking`/`allocated`) for a non-holding one releases ALL the order's leftover allocations with RESERVE-release ledger rows (moving between holding statuses — e.g. `pending → allocated`, the lock action — preserves them); leaving `shipped` clears `shipped_at`/`shipped_by` (entering stamps them), leaving `issue` clears the issue fields. 400 `invalid_status`, 404 `picking_order_not_found`; `changed: false` no-op when already the target. On change: audit transition log, `picking_order.updated` SSE event, background recompute scheduled. The list page's batch action loops this route per id. Specs `docs/superpowers/specs/2026-09-17-admin-picking-status-override-design.md`, `docs/superpowers/specs/2026-09-22-picking-allocated-skip-status-design.md` |
...
| `GET /admin/receiving-orders/:id/shipper` | Admin console: shipper `.xlsx` attachment — receipts grouped by part, each group one merged block (one bottom-aligned `invoice_no ctn_no` item row per carton; live mode: one slot column per distinct picking order — all customers on the block's first row, all order refs on the second, each carton's allocation qty in its order's column on its own row, repeat same-order allocations summed; finished mode overlays customers/refs/qtys on the block's last three rows) with per-block allocation slots; the group header cell shows the related-order allocated breakdown by source lot (`[shelf]-[date_code]/[qty]` entries, date_code omitted when NULL; Σ allocations of the part on the picking orders tracing back to this receiving order, SHELF STOCK only — receiving/dock sources are excluded, which also covers the own-order exclusion since those are already the block's slot columns; stock lots stay even when lot-traced to this batch — spec `docs/superpowers/specs/2026-09-16-admin-receiving-shipper-related-allocated-design.md`, stock-only scope + format refined by the split spec); Total/Balance per group; whole-order (no `ctn_no`) allocations close each group on an `(order-level)` block. Read-only — no in-request recompute (use `POST /admin/receiving-orders/:id/reallocate` first if stale). `?mode=finished` (for `clear` orders): same layout, slots from actual `picking_packages` (direct receiving-item picks + lot-traced via `inventory_lot_sources`) instead of live allocations, no group header cell. Split by location is the only mode: one xlsx per receiving-office `(receiving_invoice_items.org_id, sub_inventory_code)` section — a zip (`shipper-<batchNo>.zip`, members `shipper-<batchNo>-org<orgId>-<subInventoryCode|no-subinventory>.xlsx`) when the order spans >1 section, the plain xlsx under the renderer's file name when it spans one (an itemless order falls back to the combined document); whole-order/package slots attribute by the picking order's `(org_id, sub_inventory_code)` pair, related rows by the source lot's own pair, both falling back to the part's first section (spec `docs/superpowers/specs/2026-09-21-shipper-split-by-location-design.md`). Spec `docs/superpowers/specs/2026-09-14-admin-receiving-shipper-download-design.md`. |
...
separator), plus `[coo_short]` `[cow_short]` rendering the single char from
`country_list.short_code` (spec `2026-09-22-coo-cow-short-code-design.md`;
unmapped/NULL short code falls back to the raw code),
applied client-side in the admin receiving order detail and
