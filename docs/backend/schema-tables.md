...
default `"[date_code][coo]"`), `receivingOrderNameTemplate` (receiving
...
default `"[batch_no]"`), and `pdaListTemplates` (per-PDA-list `{title, meta}`
row display templates served via `GET /config` — spec
