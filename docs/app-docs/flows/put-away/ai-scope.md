...
- The put-away flow is implemented API-first in `apps/api`; the web pages above still run on PGlite until a future frontend-migration plan wires them to these endpoints.
...
- API: no UI pages are wired to the API put-away endpoints yet; they are exercised via HTTP only.
