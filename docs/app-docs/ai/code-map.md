...
| Picking orders list / detail (list is unscoped — ignores `allowedOrgIds` + user sub-inventory scope; detail page: delivery-date edit, picking-list xlsx download, per-item remove-allocation + availability search) | `/picking-orders`, `/picking-orders/:id` | `apps/admin/pages/picking-orders/index.vue`, `apps/admin/pages/picking-orders/[id].vue` |
