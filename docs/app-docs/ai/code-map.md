...
| Shelf / shelf-box label printing (single + shelf-box multi-select dialog; shelf labels use the `katata-label` template via the backend `/print/*` proxy; shelf-box labels are rendered client-side — QR + box id text + Code 128 barcode of the same value — and printed via `POST /print/files`; shelf multi-select / Print all opens an A4 batch sheet — 3 x 4 grid, QR + shelf code + zone, browser print) | `/shelves`, `/shelf-boxes` | `apps/admin/components/PrintLabelsDialog.vue`, `apps/admin/components/ShelfBoxPrintDialog.vue`, `apps/admin/components/ShelfBatchPrintDialog.vue`, `apps/admin/utils/print.ts` |
