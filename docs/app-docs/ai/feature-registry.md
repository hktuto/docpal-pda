...
| Pre-printed box ids (scan-to-create-box) | Picking | Shipped | `pages/picking/[id].vue`, `createShippingBox` `boxId` in `apps/backend/src/db/picking.ts`; box Print buttons are placeholders pending backend printing | [ai-scope](../flows/picking/ai-scope.md) |
