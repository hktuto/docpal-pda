...
畫面底部顯示此裝置目前連接的**伺服器位址**；點按**更改伺服器**可開啟伺服器選擇畫面（見下文）。

## 選擇伺服器畫面

首次啟動應用程式時（尚未選擇伺服器），會先顯示伺服器選擇畫面；之後每次啟動都會直接連到已選擇的伺服器。

![選擇伺服器畫面](./assets/server-picker.png)

- 列出五個地區伺服器：**香港、深圳、上海、廣州、北京**（開發版本會額外顯示**本機（開發）**）。
- 目前已選擇的伺服器會以綠色框標示。
- 點按任何一個伺服器即會儲存選擇並立即連線 — 應用程式會記住此選擇，無需重新安裝。

如需更換伺服器，可在登入畫面點按**更改伺服器**回到此畫面。當伺服器無法連線時，可在維護畫面按**立即重試**重新連線已選擇的伺服器，亦可按畫面上的**選擇伺服器**按鈕回到此畫面更換。
