...
- **狀態篩選** — 全部、待處理、已收貨、已完成。
- **搜尋** — 按參考編號或供應商篩選。
- 每行顯示訂單參考編號、供應商 · 交貨日期、狀態徽章及尚餘項目數量。
- 狀態篩選及搜尋列會固定在頂部，捲動長列表時保持可見。
- 點按該行開啟收貨詳情。
...
- 發票區段以緊湊行列出每個項目：料號、預期數量及 PO / 行號；發票有箱號時項目會按箱號分組（無箱號的項目歸入「無箱號」組）。點按該行可展開數量進度（預期 / 預留 / 已揀 / 已上架 / 可用、日期 / 批次 / 產地）及操作按鈕。
- **匯報問題** — 標記項目的異常（見下文）。
...
- **搜尋** — 按單號、PO 或客戶篩選；搜尋列固定在頂部，捲動時保持可見。
- 每行顯示揀貨單參考編號、客戶 · PO、日期、狀態及收貨方。
- **核取方塊**可選擇多張訂單作批量問題匯報。
- 點按該行開啟揀貨詳情。
...
- **項目** — 每行顯示料號、狀態徽章及需求 / 已掃描 / 已裝箱數量；點按該行展開分配來源及裝箱操作。
...
- 列出已裝有包裹、等待核對及量度的**運輸箱**（已開啟狀態） — 不再有測量任務；一箱可裝多張揀貨單的項目，該行會列出相關單號。
- 每行顯示箱號、狀態、相關單號及包裹核對進度；點按該行開啟測量箱號頁。
...
- 每行顯示箱號、狀態、相關單號、目的地及包裹覆核進度；點按該行開啟覆核箱號頁。
...
![覆核完成](./assets/verify-box-done.png)
